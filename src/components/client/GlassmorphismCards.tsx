...
const generateActivityPool = () => {
  ...
  for (let i = 1; i <= 500; i++) {
    ...
    do {
      const firstName =
        nameGroup.firstNames[
          Math.floor(Math.random() * nameGroup.firstNames.length)
        ];
      const lastName =
        nameGroup.lastNames[
          Math.floor(Math.random() * nameGroup.lastNames.length)
        ];
      ...
      if (attempts > 20) {
        const altGroup =
          nameGroups[Math.floor(Math.random() * nameGroups.length)];
        const altFirstName =
          altGroup.firstNames[
            Math.floor(Math.random() * altGroup.firstNames.length)
          ];
        const altLastName =
          altGroup.lastNames[
            Math.floor(Math.random() * altGroup.lastNames.length)
          ];
    ...
    if (type === "deposit") {
      ...
      const currency =
        nameGroup.currencies[
          Math.floor(Math.random() * nameGroup.currencies.length)
        ];
      const value = Math.floor(
        Math.random() * (currency.max - currency.min) + currency.min,
      );
...
export default function GlassmorphismCards() {
    ...
    const scheduleNextActivity = () => {
      ...
      setTimeout(() => {
        setUsedIndices((prevIndices) => {
          ...
          const indices =
            prevIndices.size >= activityPool.length
              ? new Set<number>()
              : new Set(prevIndices);
  ...
  return (
    <>
      ...
      <div className="absolute left-4 top-20 z-[6] hidden xl:block">
        <motion.div
        ...
        >
          ...
          <div className="space-y-1.5">
            {recentActivities.map((activity) => (
              <div
                key={activity.id}
                className={`flex items-center justify-between text-xs rounded px-2 py-1 transition-all duration-300 ${
      ...
      <div className="absolute right-4 top-36 z-[6] hidden xl:block space-y-3">
